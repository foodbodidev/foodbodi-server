...
License.prototype.validateInput = function(input, create) {
    ...
    if (create) {
        ...
        if (boss_id == null) return "Missing boss id";
        ...
        if (status != null) return "Not allow to set status"
    }
    if (!!business_name && typeof name !== "string") return "Business name must be a string";
    if (!!restaurant_id && !validator.isAlphanumeric(restaurant_id)) return "Restaurant id is invalid";
    if (!!boss_id && typeof !validator.isAlphanumeric(restaurant_id)) return "Boss id invalid";
